    ...
    global.commChallenge = {
        weekKey() { try { return quests.weekKey(); } catch (e) { return ''; } },
            ...
            if (!game.comm || game.comm.week !== wk) {
                ...
                game.comm = { week: wk, prog: 0, goal: Math.max(balanceGet('commGoalMin', 4500000), Math.floor(cps * balanceGet('commGoalCpsMult', 7200))), done: false, buffUntil: buffUntil };
        ...
        dropMult() {
            if (game.comm && game.comm.buffUntil > Date.now()) return balanceGet('commWeeklyDropMult', 1.12);
            return 1;
        },
        pct() {
    ...
    global.soupBoss = {
        weekKey() { try { return quests.weekKey(); } catch (e) { return ''; } },
            ...
            if (!game.boss || game.boss.week !== wk) {
                let cps = 100;
                try { cps = Math.max(100, game.getCps()); } catch (e) {}
                const max = Math.max(balanceGet('bossHpMin', 350000), Math.floor(cps * balanceGet('bossHpCpsMult', 420)));
        ...
        rewardPreview() {
            let cps = 100;
            try { cps = Math.max(100, game.getCps()); } catch (e) {}
            return Math.max(balanceGet('bossRewardMin', 18000), Math.floor(cps * balanceGet('bossRewardCpsMult', 200)));
        },
        defeated() { this.ensure(); return !!(game.boss && game.boss.hp <= 0); },
        clickDamage() {
            let cps = 100;
            try { cps = Math.max(100, game.getCps()); } catch (e) {}
            return Math.max(balanceGet('bossClickDmgMin', 90), cps * balanceGet('bossClickDmgCpsMult', 0.52));
        },
        damage(amt) {
            this.ensure();
        ...
        idleTick(cps) {
            this.ensure();
            if (!game.boss || game.boss.claimed || game.boss.hp <= 0) return;
            const rate = balanceGet('bossIdleDpsCpsMult', 0.065);
            const dmg = Math.max(0, (Number(cps) || 0) * rate);
        ...
        claim() {
            ...
            let cps = 100;
            try { cps = Math.max(100, game.getCps()); } catch (e) {}
            const reward = Math.max(balanceGet('bossRewardMin', 18000), Math.floor(cps * balanceGet('bossRewardCpsMult', 200)));
            game.e += reward;
            game.te += reward;
            game.as += balanceGet('bossRewardShard', 1);
